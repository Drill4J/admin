...
class RisksApiTest : MetricsDatabaseTests({ default, metrics ->
    ...
    @Test
    fun `given isolated tested target build, risks service should return changes with coverage in current build`(): Unit =
        havingData {
            initBuildsAndMethodsData()
            test1 covers method2 with probesOf(1, 1, 0) on build3
        }.expectThat {
            client.get("/metrics/risks") {
    ...
    @Test
    fun `given aggregated tested builds, risks service should return changes with coverage in other builds`(): Unit =
        havingData {
            initBuildsAndMethodsData()
