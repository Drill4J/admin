WITH request_coverage AS (
    SELECT
        ...
        c.probes,
        m.signature
    FROM raw_data.test_launch_coverage_requests req
    JOIN raw_data.test_launches tl ON tl.test_session_id = req.test_session_id
        AND (req.test_definition_id IS NULL OR req.test_definition_id = '' OR tl.test_definition_id = req.test_definition_id)
        AND tl.group_id = req.group_id
    ...
    ORDER BY req.created_at, tl.created_at, c.created_at, c.method_id
    LIMIT :limit
)
SELECT
    c.req_created_at,
...
ORDER BY c.req_created_at, c.test_completed_at, c.created_at, c.method_id