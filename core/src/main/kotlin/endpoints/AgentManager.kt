...
class AgentManager(override val di: DI) : DIAware {
    ...
    /**
     * Ensure all plugin instances
     * @param agent the agent
     * @receiver the plugin IDs
     *
     */
    ...
    private suspend fun ensurePluginInstance(
        ...
        agent[pluginId] ?: agent.get(pluginId) {
            ...
            plugin.createInstance(
                agentInfo = info,
                data = agentData,
                sender = pluginSenders.sender(plugin.pluginBean.id), //todo why don't to change plugin.pluginBean.id to pluginId ?
                store = pluginStore
            )
        }.apply {
...
/**
 * Initialize class loading from the agent
 *
 * @features Agent registration
 */
 ...
 *
 * @param agentWsSession Current agent WebSocket session
